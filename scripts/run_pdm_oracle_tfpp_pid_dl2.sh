...
LIMIT=${LIMIT:-20}
RUN_NAME=${RUN_NAME:-eval_pdm_oracle_tfpp_pid_tesla_20260814}
...
start_carla() {
  ...
  for _ in $(seq 1 75); do
    if ! kill -0 "${SERVER_PID}" 2>/dev/null; then
      echo "CARLA exited during startup; see ${HOST_RUN_ROOT}/carla_logs/carla.log" >&2
      return 1
    fi
    if timeout 1 bash -c "</dev/tcp/127.0.0.1/${PORT}" 2>/dev/null; then
      sleep 12
  ...
  echo "CARLA did not open port ${PORT}" >&2
  return 1
}

echo "[$(date --iso-8601=seconds)] PDM oracle -> TF++ PID evaluation starting GPU=${GPU}"
start_carla
...
docker run --rm --network host --gpus "device=${GPU}" \
  ...
  -e STOP_ON_TIMEOUT=0 \
  -e STOP_ON_INVALID=0 \
  -e INVALID_RETRY_LIMIT=2 \
  -e CARLA_WATCHDOG=1 \
  -e CLEANUP_AFTER_MISSION=1 \
  -e RECORD_VIDEO=1 \
  -e VIDEO_RECORD_INDICES="${VIDEO_RECORD_INDICES}" \
  -e VIDEO_VIEW=topdown \
  -e VIDEO_PIP_VIEW=front \
  -e VIDEO_NOTION_COMPAT=1 \
  -e EGO_VEHICLE_MODEL=vehicle.tesla.model3 \
  -e AGENT_PATH="${CONTAINER_ADAPTER_ROOT}/scripts/tfpp_pdm_oracle_pid_agent.py" \
  -e TFPP_ORACLE_TRACE_PATH="${CONTAINER_RUN_DIR}/traces/oracle_pid.jsonl" \
...
echo "[$(date --iso-8601=seconds)] PDM oracle -> TF++ PID evaluation complete"
