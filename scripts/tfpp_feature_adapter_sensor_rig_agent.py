#!/usr/bin/env python3
"""TransFuser++ leaderboard agent with shifted sensors and fused-feature adapter."""
...
def get_entry_point() -> str:
    return "FeatureAdapterSensorRigAgent"
...
class FeatureAdapterSensorRigAgent(SensorAgent):
    ...
    def _apply_sensor_rig(self) -> None:
        ...
        print(
            "[FeatureAdapterSensorRigAgent] applied rig="
    ...
    def _load_feature_adapter(self) -> None:
        checkpoint_path = (
            os.environ.get("TFPP_FEATURE_ADAPTER_CHECKPOINT")
            or os.environ.get("TFPP_ADAPTER_CHECKPOINT")
            or ""
        )
        if not checkpoint_path:
            raise ValueError("TFPP_FEATURE_ADAPTER_CHECKPOINT or TFPP_ADAPTER_CHECKPOINT is required")
        _ensure_adapter_import_path()
        from teach2drive_adapter.train_transfuserpp_fused_feature_adapter import ResidualFusedFeatureAdapter

        checkpoint = torch.load(Path(checkpoint_path).expanduser(), map_location=self.device)
        metadata = checkpoint.get("metadata", {})
        args = metadata.get("args", {})
        feature_shape = tuple(int(v) for v in checkpoint.get("feature_shape", metadata.get("feature_shape", [])))
        if len(feature_shape) != 3:
            raise ValueError(f"Invalid feature adapter shape in checkpoint: {feature_shape}")
        self._feature_adapter = ResidualFusedFeatureAdapter(
            channels=int(feature_shape[0]),
            hidden_channels=int(args.get("hidden_channels", 0)),
            blocks=int(args.get("blocks", 2)),
            dropout=float(args.get("dropout", 0.0)),
        ).to(self.device)
        missing, unexpected = self._feature_adapter.load_state_dict(checkpoint["model_state"], strict=False)
        self._feature_adapter.eval()
        self._feature_adapter_blend = _env_float("TFPP_FEATURE_ADAPTER_BLEND", 1.0)
        self._feature_adapter_shape = feature_shape
        print(
            "[FeatureAdapterSensorRigAgent] loaded fused feature adapter "
            f"checkpoint={Path(checkpoint_path).expanduser()} shape={feature_shape} "
            f"blend={self._feature_adapter_blend:.3f} missing={len(missing)} unexpected={len(unexpected)}",
            flush=True,
        )
    ...
    def _patch_backbones(self) -> None:
        for index, net in enumerate(self.nets):
            ...
            def adapted_forward(*args, _original_forward=original_forward, _index=index, **kwargs):
                ...
                if fused.ndim != 4:
                    return output
                expected = self._feature_adapter_shape
                if tuple(int(v) for v in fused.shape[1:]) != expected:
                    if getattr(self, "_feature_adapter_shape_warned", False) is False:
                        print(
                            "[FeatureAdapterSensorRigAgent] fused feature shape mismatch "
                            f"net={_index} got={tuple(fused.shape[1:])} expected={expected}; skipping adapter",
                            flush=True,
                        )
                        self._feature_adapter_shape_warned = True
                    return output
                adapted = self._feature_adapter(fused.float()).to(dtype=fused.dtype)
                blend = float(self._feature_adapter_blend)
                if blend < 1.0:
        ...
        print(f"[FeatureAdapterSensorRigAgent] patched {len(self.nets)} backbone(s)", flush=True)
    ...
    def _setup_recording(self) -> None:
        ...
        if self._record_enabled and self._record_output:
            ...
            print(
                "[FeatureAdapterSensorRigAgent] recording "
    ...
    def _close_recording(self) -> None:
        ...
        print(
            f"[FeatureAdapterSensorRigAgent] saved_frames={self._record_frames} "
