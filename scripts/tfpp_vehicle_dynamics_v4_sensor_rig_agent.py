#!/usr/bin/env python3
"""TF++ agent with lag-aware dynamics and future-pose yaw calibration v4."""
...
def get_entry_point() -> str:
    return "VehicleDynamicsV4SensorRigAgent"


class VehicleDynamicsV4SensorRigAgent(SensorRigAgent):
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        from teach2drive_adapter.vehicle_dynamics_v4 import HorizonYawRateCalibrator, InverseDynamicsV4Spec

        dynamics_payload = torch.load(Path(os.environ["TFPP_VEHICLE_DYNAMICS_V3_CHECKPOINT"]).expanduser(), map_location="cpu")
        self._vehicle_dynamics = LaggedSpeedConditionedYawDynamics()
        self._vehicle_dynamics.load_state_dict(dynamics_payload["lagged_vehicle_dynamics_state"])
        self._vehicle_dynamics.eval()

        calibrator_path = Path(os.environ["TFPP_YAW_HORIZON_V4_CHECKPOINT"]).expanduser()
        calibrator_payload = torch.load(calibrator_path, map_location="cpu")
        calibrator_metadata = calibrator_payload.get("metadata", {}).get("horizon_yaw_calibrator", {})
        self._yaw_calibrator = HorizonYawRateCalibrator(
            horizons=len(calibrator_metadata.get("horizons_seconds", [0.25, 0.50, 0.75])),
            minimum_gain=float(calibrator_metadata.get("minimum_gain", 0.85)),
            maximum_gain=float(calibrator_metadata.get("maximum_gain", 1.10)),
        )
        self._yaw_calibrator.load_state_dict(calibrator_payload["horizon_yaw_calibrator_state"])
        self._yaw_calibrator.eval()

        self._inverse_spec = InverseDynamicsV4Spec(
        ...
        self._dynamics_blend = float(os.environ.get("TFPP_DYNAMICS_BLEND", "0.25"))
        ...
        self._steer_commands: deque[float] = deque(maxlen=max(self._lag2_frames + 2, 16))
        ...
        print(
            f"[VehicleDynamicsV4] calibrator={calibrator_path} blend={self._dynamics_blend:.3f} "
    ...
    def _patch_lateral_controller(self) -> None:
        from teach2drive_adapter.vehicle_dynamics_v4 import bounded_horizon_inverse_steer_v4
