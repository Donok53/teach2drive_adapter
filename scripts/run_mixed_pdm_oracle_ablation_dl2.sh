...
MODE=${MODE:?Set MODE=checkpoint or MODE=speed}
GPU=${GPU:-0}
...
LIMIT=${LIMIT:-20}
RUN_NAME=${RUN_NAME:-eval_mixed_oracle_${MODE}_20260815}
...
if [[ "${MODE}" != "checkpoint" && "${MODE}" != "speed" ]]; then
  echo "MODE must be checkpoint or speed" >&2
  exit 2
fi

mkdir -p "${HOST_RUN_ROOT}/carla_logs" "${HOST_RUN_ROOT}/traces" "${HOST_HF_CACHE}"
...
start_carla() {
  ...
  for _ in $(seq 1 75); do
    if ! kill -0 "${SERVER_PID}" 2>/dev/null; then
      echo "CARLA exited during startup" >&2
      return 1
    fi
    if timeout 1 bash -c "</dev/tcp/127.0.0.1/${PORT}" 2>/dev/null; then
      sleep 12
...
echo "[$(date --iso-8601=seconds)] mixed oracle mode=${MODE} starting GPU=${GPU}"
start_carla
...
docker run --rm --network host --gpus "device=${GPU}" \
  ...
  -e AGENT_PATH="${CONTAINER_ADAPTER_ROOT}/scripts/tfpp_mixed_pdm_oracle_sensor_rig_agent.py" \
  -e TFPP_SENSOR_RIG=tfpp_ego -e TFPP_SENSOR_CAMERA=front -e TFPP_SENSOR_LIDAR=top \
  -e TFPP_ORACLE_MIX_MODE="${MODE}" \
  -e TFPP_ORACLE_TRACE_PATH="${CONTAINER_RUN_DIR}/traces/mixed_oracle_${MODE}.jsonl" \
...
echo "[$(date --iso-8601=seconds)] mixed oracle mode=${MODE} complete"
