...
if path.exists():
    with path.open("r", encoding="utf-8") as handle:
        for row in csv.DictReader(handle, delimiter="\t"):
            try: best = max(best, int(row["index"]))
            except (KeyError, TypeError, ValueError): pass
print(best + 1)
PY
  )
  if (( next_index >= TOTAL )); then
    echo "[$(date --iso-8601=seconds)] evaluation complete rows=${next_index}/${TOTAL}"
    exit 0
  fi
  echo "[$(date --iso-8601=seconds)] resume=${attempt} start=${next_index} remaining=$((TOTAL-next_index))"
  set +e
  (cd "${ADAPTER_ROOT}" && env GPU="${GPU}" PORT="${PORT}" TM_PORT="${TM_PORT}" \
    RUN_NAME="${RUN_NAME}" CHECKPOINT="${CHECKPOINT}" START_INDEX="${next_index}" \
    LIMIT="$((TOTAL-next_index))" VIDEO_RECORD_INDICES=0,1,2,3,4 \
    bash scripts/run_policy_preserving_adapter_eval_dl2.sh)
  status=$?
  set -e
