...
ROUTE_END="${ROUTE_END:-385}"
ROUTE_TIMEOUT_SEC="${ROUTE_TIMEOUT_SEC:-720}"
ROUTE_TIMEOUT_KILL_AFTER_SEC="${ROUTE_TIMEOUT_KILL_AFTER_SEC:-60}"
MAX_ROUTE_ATTEMPTS="${MAX_ROUTE_ATTEMPTS:-1000}"
...
tmux new-session -d -s "$SESSION" bash -lc \
  "cd '$PWD' && \
   ...
   ROUTE_TIMEOUT_KILL_AFTER_SEC='$ROUTE_TIMEOUT_KILL_AFTER_SEC' \
   bash configs/collect_tesla_pdm_lite_8town_3h_supervised_autocarla_server.sh"

echo "started collection session=$SESSION port=$PORT tm_port=$TM_PORT gpu=$CARLA_GRAPHICS_ADAPTER quality=$CARLA_QUALITY_LEVEL args=$CARLA_EXTRA_ARGS route=${ROUTE_START}-${ROUTE_END} timeout=${ROUTE_TIMEOUT_SEC}s log=$RUN_LOG"
tmux ls
