...
# v3.2 preset: canonical trajectory teacher + expert speed/stop labels.
# v3/v3.1 recovered route following but inherited too much teacher speed, which
# made junction/signal behavior aggressive. This keeps the canonical teacher for
# geometry, while using the collected expert labels for speed and stop targets.
...
export WORK_ROOT=${WORK_ROOT:-"$HOME/teach2drive/runs/tfpp_shifted_adapter_motion_safe_e2e_v32_teacher_traj_expert_speed"}
...
export OUT=${OUT:-"$WORK_ROOT/train_shifted_visual_layout_adapter_teacher_distill_v32"}
...
# Split target blending by output region. Keep route geometry close to canonical
# TransFuser++, but do not let the fast teacher overwrite expert speed/stop.
...
# Small target-consistency losses reduce stop-go oscillation without changing
# inference-time policy logic.
...
export STOP_STATE_LOSS_WEIGHT=${STOP_STATE_LOSS_WEIGHT:-0.08}
