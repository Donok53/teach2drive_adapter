...
export SPLIT_MODE=${SPLIT_MODE:-episode}
export VAL_REQUIRED_STOP_REASONS=${VAL_REQUIRED_STOP_REASONS:-""}
export VAL_MIN_REASON_SAMPLES=${VAL_MIN_REASON_SAMPLES:-1}
...
export DIRECT_CONTROL_LOSS_WEIGHT=${DIRECT_CONTROL_LOSS_WEIGHT:-0.0}
export DIRECT_CONTROL_COACTIVATION_LOSS_WEIGHT=${DIRECT_CONTROL_COACTIVATION_LOSS_WEIGHT:-0.0}
...
export CONTROLLER_GO_HINGE_LOSS_WEIGHT=${CONTROLLER_GO_HINGE_LOSS_WEIGHT:-0.0}
export CONTROLLER_THROTTLE_CLOSE_HINGE_LOSS_WEIGHT=${CONTROLLER_THROTTLE_CLOSE_HINGE_LOSS_WEIGHT:-0.0}
export PDM_GO_PROGRESS_HINGE_LOSS_WEIGHT=${PDM_GO_PROGRESS_HINGE_LOSS_WEIGHT:-0.0}
export CLOSED_LOOP_UNROLL_LOSS_WEIGHT=${CLOSED_LOOP_UNROLL_LOSS_WEIGHT:-0.0}
export CLOSED_LOOP_UNROLL_STEPS=${CLOSED_LOOP_UNROLL_STEPS:-6}
export CLOSED_LOOP_UNROLL_DT=${CLOSED_LOOP_UNROLL_DT:-0.5}
export CLOSED_LOOP_UNROLL_STEER_GAIN=${CLOSED_LOOP_UNROLL_STEER_GAIN:-1.35}
export CLOSED_LOOP_UNROLL_MAX_SPEED_MPS=${CLOSED_LOOP_UNROLL_MAX_SPEED_MPS:-8.0}
export CLOSED_LOOP_UNROLL_TARGET_SPEED_BLEND=${CLOSED_LOOP_UNROLL_TARGET_SPEED_BLEND:-0.0}
export SAFE_CTRL_SURROGATE_LOSS_WEIGHT=${SAFE_CTRL_SURROGATE_LOSS_WEIGHT:-0.0}
export SAFE_CTRL_SURROGATE_TEMPERATURE=${SAFE_CTRL_SURROGATE_TEMPERATURE:-0.25}
...
export OUTPUT_RESIDUAL_GATE_MAX=${OUTPUT_RESIDUAL_GATE_MAX:-1.0}
export OUTPUT_RESIDUAL_DROPOUT=${OUTPUT_RESIDUAL_DROPOUT:-0.0}
export OUTPUT_RESIDUAL_CONTROLLER=${OUTPUT_RESIDUAL_CONTROLLER:-0}
export OUTPUT_RESIDUAL_CONTROLLER_LATERAL_SCALE=${OUTPUT_RESIDUAL_CONTROLLER_LATERAL_SCALE:-1.0}
export OUTPUT_RESIDUAL_CONTROLLER_PROGRESS_SCALE=${OUTPUT_RESIDUAL_CONTROLLER_PROGRESS_SCALE:-1.0}
export OUTPUT_RESIDUAL_CONTROLLER_SPEED_LOGIT_SCALE=${OUTPUT_RESIDUAL_CONTROLLER_SPEED_LOGIT_SCALE:-2.0}
export OUTPUT_RESIDUAL_CONTROLLER_GATE_BIAS=${OUTPUT_RESIDUAL_CONTROLLER_GATE_BIAS:--2.0}
export OUTPUT_RESIDUAL_CONTROLLER_GATE_MAX=${OUTPUT_RESIDUAL_CONTROLLER_GATE_MAX:-1.0}
export OUTPUT_RESIDUAL_GATE_LOSS_WEIGHT=${OUTPUT_RESIDUAL_GATE_LOSS_WEIGHT:-0.0}
export OUTPUT_RESIDUAL_NORM_LOSS_WEIGHT=${OUTPUT_RESIDUAL_NORM_LOSS_WEIGHT:-0.0}
export FREEZE_TASK_ADAPTER=${FREEZE_TASK_ADAPTER:-0}
...
export TRAFFICLIGHT_SAMPLE_WEIGHT=${TRAFFICLIGHT_SAMPLE_WEIGHT:-1.0}
export STOPSIGN_SAMPLE_WEIGHT=${STOPSIGN_SAMPLE_WEIGHT:-1.0}
export FRONTVEHICLE_SAMPLE_WEIGHT=${FRONTVEHICLE_SAMPLE_WEIGHT:-1.0}
export JUNCTIONYIELD_SAMPLE_WEIGHT=${JUNCTIONYIELD_SAMPLE_WEIGHT:-1.0}
...
export TRAFFICLIGHT_STOP_SPEED_CEILING_LOSS_WEIGHT=${TRAFFICLIGHT_STOP_SPEED_CEILING_LOSS_WEIGHT:-0.0}
export STOPSIGN_STOP_SPEED_CEILING_LOSS_WEIGHT=${STOPSIGN_STOP_SPEED_CEILING_LOSS_WEIGHT:-0.0}
export FRONTVEHICLE_STOP_SPEED_CEILING_LOSS_WEIGHT=${FRONTVEHICLE_STOP_SPEED_CEILING_LOSS_WEIGHT:-0.0}
export JUNCTIONYIELD_STOP_SPEED_CEILING_LOSS_WEIGHT=${JUNCTIONYIELD_STOP_SPEED_CEILING_LOSS_WEIGHT:-0.0}
...
export CONTROLLER_GO_HINGE_MARGIN=${CONTROLLER_GO_HINGE_MARGIN:-0.02}
export CONTROLLER_SOFT_STOP_TEMPERATURE=${CONTROLLER_SOFT_STOP_TEMPERATURE:-1.0}
...
if [[ "$OUTPUT_RESIDUAL_CONTROLLER" == "1" || "$OUTPUT_RESIDUAL_CONTROLLER" == "true" || "$OUTPUT_RESIDUAL_CONTROLLER" == "TRUE" ]]; then
  TRAIN_ARGS+=(--output-residual-controller)
fi
if [[ "$FREEZE_TASK_ADAPTER" == "1" || "$FREEZE_TASK_ADAPTER" == "true" || "$FREEZE_TASK_ADAPTER" == "TRUE" ]]; then
  TRAIN_ARGS+=(--freeze-task-adapter)
fi
...
PYTHONUNBUFFERED=1 "$PY" -m teach2drive_adapter.train_transfuserpp_task_feature_adapter \
  ...
  --val-required-stop-reasons "$VAL_REQUIRED_STOP_REASONS" \
  --val-min-reason-samples "$VAL_MIN_REASON_SAMPLES" \
  --seed "$SEED" \
  ...
  --direct-control-loss-weight "$DIRECT_CONTROL_LOSS_WEIGHT" \
  --direct-control-coactivation-loss-weight "$DIRECT_CONTROL_COACTIVATION_LOSS_WEIGHT" \
  ...
  --controller-go-hinge-loss-weight "$CONTROLLER_GO_HINGE_LOSS_WEIGHT" \
  --controller-throttle-close-hinge-loss-weight "$CONTROLLER_THROTTLE_CLOSE_HINGE_LOSS_WEIGHT" \
  --pdm-go-progress-hinge-loss-weight "$PDM_GO_PROGRESS_HINGE_LOSS_WEIGHT" \
  --closed-loop-unroll-loss-weight "$CLOSED_LOOP_UNROLL_LOSS_WEIGHT" \
  --closed-loop-unroll-steps "$CLOSED_LOOP_UNROLL_STEPS" \
  --closed-loop-unroll-dt "$CLOSED_LOOP_UNROLL_DT" \
  --closed-loop-unroll-steer-gain "$CLOSED_LOOP_UNROLL_STEER_GAIN" \
  --closed-loop-unroll-max-speed-mps "$CLOSED_LOOP_UNROLL_MAX_SPEED_MPS" \
  --closed-loop-unroll-target-speed-blend "$CLOSED_LOOP_UNROLL_TARGET_SPEED_BLEND" \
  --safe-ctrl-surrogate-loss-weight "$SAFE_CTRL_SURROGATE_LOSS_WEIGHT" \
  --safe-ctrl-surrogate-temperature "$SAFE_CTRL_SURROGATE_TEMPERATURE" \
  ...
  --output-residual-gate-max "$OUTPUT_RESIDUAL_GATE_MAX" \
  --output-residual-dropout "$OUTPUT_RESIDUAL_DROPOUT" \
  --output-residual-controller-lateral-scale "$OUTPUT_RESIDUAL_CONTROLLER_LATERAL_SCALE" \
  --output-residual-controller-progress-scale "$OUTPUT_RESIDUAL_CONTROLLER_PROGRESS_SCALE" \
  --output-residual-controller-speed-logit-scale "$OUTPUT_RESIDUAL_CONTROLLER_SPEED_LOGIT_SCALE" \
  --output-residual-controller-gate-bias "$OUTPUT_RESIDUAL_CONTROLLER_GATE_BIAS" \
  --output-residual-controller-gate-max "$OUTPUT_RESIDUAL_CONTROLLER_GATE_MAX" \
  --output-residual-gate-loss-weight "$OUTPUT_RESIDUAL_GATE_LOSS_WEIGHT" \
  --output-residual-norm-loss-weight "$OUTPUT_RESIDUAL_NORM_LOSS_WEIGHT" \
  ...
  --trafficlight-sample-weight "$TRAFFICLIGHT_SAMPLE_WEIGHT" \
  --stopsign-sample-weight "$STOPSIGN_SAMPLE_WEIGHT" \
  --frontvehicle-sample-weight "$FRONTVEHICLE_SAMPLE_WEIGHT" \
  --junctionyield-sample-weight "$JUNCTIONYIELD_SAMPLE_WEIGHT" \
  ...
  --trafficlight-stop-speed-ceiling-loss-weight "$TRAFFICLIGHT_STOP_SPEED_CEILING_LOSS_WEIGHT" \
  --stopsign-stop-speed-ceiling-loss-weight "$STOPSIGN_STOP_SPEED_CEILING_LOSS_WEIGHT" \
  --frontvehicle-stop-speed-ceiling-loss-weight "$FRONTVEHICLE_STOP_SPEED_CEILING_LOSS_WEIGHT" \
  --junctionyield-stop-speed-ceiling-loss-weight "$JUNCTIONYIELD_STOP_SPEED_CEILING_LOSS_WEIGHT" \
  ...
  --controller-go-hinge-margin "$CONTROLLER_GO_HINGE_MARGIN" \
  --controller-soft-stop-temperature "$CONTROLLER_SOFT_STOP_TEMPERATURE" \
  --grad-clip "$GRAD_CLIP" \
  "${TRAIN_ARGS[@]}" \
