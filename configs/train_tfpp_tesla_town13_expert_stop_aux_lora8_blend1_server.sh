...
# Expert-only adapter plus auxiliary stop_state / stop_reason supervision.
# The auxiliary heads are training-only regularizers on adapted fused features.
...
export WORK_ROOT=${WORK_ROOT:-"$HOME/dataset/byeongjae/runs/tfpp_tesla_town13_expert_stop_aux_lora8_blend1"}
export OUT=${OUT:-"$WORK_ROOT/train_front_triplet_shifted_expert_stop_aux_lora8_blend1"}
...
export STOP_STATE_AUX_LOSS_WEIGHT=${STOP_STATE_AUX_LOSS_WEIGHT:-0.15}
export STOP_REASON_AUX_LOSS_WEIGHT=${STOP_REASON_AUX_LOSS_WEIGHT:-0.10}
