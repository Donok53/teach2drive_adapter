...
RUN_DIR=${RUN_DIR:-/data/dataset/byeongjae/runs/train_v4_gist3h_teacher_w05}
...
echo "=== v3 recipe on GIST 3h, backbone FROZEN + geometric-teacher distillation (stage-adapter=all, distill-w=0.5, es4) start $(date '+%F %T') ==="
...
python -m teach2drive_adapter.train_transfuserpp_task_feature_adapter \
  ...
  --max-train-samples 0 \
  --max-val-samples 0 \
  --teacher-view-root /data/dataset/byeongjae/datasets/pdm_lite_front_triplet_shifted_3h_subset/data \
  --teacher-view-dirname rgb_front_teacher_xm15 \
  --step-log-every 50
