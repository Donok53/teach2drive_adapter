...
CHECKPOINT=${CHECKPOINT:?Set CHECKPOINT to the trained best_model.pt}
GPU=${GPU:-1}
PORT=${PORT:-2043}
...
echo "=== exact adapter target20 eval start $(date '+%F %T') ==="
echo "run_dir=${RUN_DIR}"
echo "checkpoint=${CHECKPOINT}"
echo "gpu=${GPU} start_index=${START_INDEX} limit=${LIMIT} video_indices=${VIDEO_RECORD_INDICES}"
