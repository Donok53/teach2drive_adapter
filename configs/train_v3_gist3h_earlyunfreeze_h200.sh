#!/usr/bin/env bash
# v3 recipe on GIST 3h + PARTIAL backbone unfreeze (image encoder early layers only).
#
# Rationale
# ---------
# Verified against the official PDM-Lite dataset that the only real domain gap is the
# camera: LiDAR is stored in ego coordinates (ground at z~0, +-83m extent, ~30k pts
# median) identically to the reference data, so the LiDAR encoder has nothing to adapt.
# The camera moved 2.75m forward (x -1.50 -> +1.25), which is a low-level geometric
# change (depth-dependent scale/parallax), i.e. exactly what the *early* conv stages
# encode.
#
# Two extremes were already measured on the identical 20-mission/180s protocol:
#   full backbone unfrozen (106.7M params) -> composed 14.09  (catastrophic forgetting)
#   backbone fully frozen                  -> composed 48.59  (adapter repairs downstream)
# This run takes the middle ground that was never tried: unfreeze only
# image_encoder.{stem,s1,s2} = 858,724 params (0.7% of the backbone) so the geometry can
# be re-tuned at the point where it is actually encoded, while the semantic layers
# (s3/s4), the whole LiDAR encoder and the fusion transformers stay frozen.
#
# unfreeze_lr is kept well below the main lr so the pretrained features drift slowly.
set -u

RUN_DIR=${RUN_DIR:-/data/dataset/byeongjae/runs/train_v3_gist3h_earlyunfreeze}
...
echo "=== v3 GIST3h + early image-encoder unfreeze  start $(date '+%F %T') ==="
...
python -m teach2drive_adapter.train_transfuserpp_task_feature_adapter \
  ...
  --lora-exclude '' \
  --unfreeze-include '^backbone\.image_encoder\.(stem|s1|s2)\.' \
  --unfreeze-exclude '' \
  --unfreeze-lr 1e-06 \
  --unfreeze-weight-decay 1e-05 \
  --epochs 20 \
