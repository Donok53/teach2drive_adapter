...
# Stage 1 (turn-only): learn a bounded lateral correction to TF++'s spatial
# route checkpoints from paired canonical-camera data.  TF++ itself, the speed
# logits, longitudinal checkpoints, feature adapters, and controller remain
# unchanged.

export RUN_NAME=${RUN_NAME:-train_exact_stage1_turn_lateral_route_gpu1}
...
# Same-frame measurement["route"] is the supervision contract.  It has the
# same spatial checkpoint meaning as TF++ pred_checkpoint; traj_targets and
# future-speed/control labels are deliberately excluded from this stage.
...
export ROUTE_STRAIGHT_IDENTITY_LOSS_WEIGHT=${ROUTE_STRAIGHT_IDENTITY_LOSS_WEIGHT:-0.20}
export ROUTE_TURN_GATE_LOSS_WEIGHT=${ROUTE_TURN_GATE_LOSS_WEIGHT:-0.10}
